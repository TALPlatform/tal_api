...
type PeopleUsecaseInterface interface {
	// INJECT INTERFACE
	RawProfileSync(ctx context.Context, sessionId int32, crustDataProfiles *crustdata.PeopleSearchResponse) error
	...
	RawProfileList(ctx context.Context, req *connect.Request[talv1.RawProfileListRequest], stream *connect.ServerStream[talv1.RawProfileListResponse]) error
}
...
type RawProfileListRequestRaw struct {
	YearsOfExperienceFrom int32    `json:"years_of_experience_from,omitempty"`
	YearsOfExperienceTo   int32    `json:"years_of_experience_to,omitempty"`
...
func NewPeopleUsecase(store db.Store,
	crustDataClient crustdata.CrustdataServiceInterface,
	asynqClient asynqclient.Enqueuer, // 👈 new dependency
	// llmClient genaiclient.GenaiClientInterface
) (PeopleUsecaseInterface, error) {
	// agentName :=  "raw_profile_list_request_builder"
	// profileAgent ,err := llmClient.GetAgent(ctx,agentName)
	// if err != nil {
	// 	return nil , fmt.Errorf("error getting agent %s : %w", agentName , err)
	// }
	...
	return &PeopleUsecase{
		...
		adapter: adapter.NewPeopleAdapter(),
